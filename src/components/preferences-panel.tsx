...
export function PreferencesPanel({
  ...
  const [open, setOpen] = useState(false),
    [draft, setDraft] = useState<PoolProfile>(emptyProfile),
    [search, setSearch] = useState(""),
    [tab, setTab] = useState<"builtIn" | "custom">("builtIn");
    ...
    try {
      ...
      setNotice(
        vi
          ...
          : "Name: 1–60 characters, price: 10–500k VND, up to 50 dishes.",
      );
    }
  }
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="preferences-dialog">
          <DialogTitle>{vi ? "Món của tôi" : "My dishes"}</DialogTitle>
          <DialogDescription>
            {vi
              ...
              {tab === "builtIn" ? (
                <>
                  <input
                    ...
                    placeholder={vi ? "Tìm món…" : "Search dishes…"}
                ...
                <>
                  ...
                  <div className="pool-list">
                    {draft.custom.length === 0 && (
                      <p>
                        {vi
                          ? "Thêm quán cơm quen hoặc món tủ của bạn."
                          : "Add your favorite lunch dish."}
                      </p>
                    )}
                    {draft.custom.map((f) => (
          ...
          <div className="pool-save">
            <small>
              {foods.length - draft.disabled.length + draft.custom.length}{" "}
              {vi ? "món trong danh sách" : "dishes in your list"} ·{" "}
              {dirty
                ? vi
                  ? "Chưa lưu"
                  : "Unsaved"
                : vi
                  ? "Tự động lưu trên máy"
                  : "Saved automatically"}
            </small>
          </div>
